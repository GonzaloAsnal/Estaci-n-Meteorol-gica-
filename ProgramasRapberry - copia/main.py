...
punto_cardinal = "-"
lluvia_acumulada = 0
#variables para el guardado de datos cuando se pierde la conexion
ban_guardado = 0
cont_guardado= 0
datos_noenviado = []
...
SERVER_URL = "https://electronica.frvm.utn.edu.ar/central_meteorologica/index.php"
...
def PaginaWeb(contenido,posicion,pg):
    global conGases,conParticulas,lluvia_acumulada,velocidad_viento_actual,clima,punto_cardinal,temp,hum,conParticulas,conGases,presion,valor_uv,valor_db,horash,valoresh, horashs,valoreshs,horas7d,valores7d,horas31d,valores31d,horas12m,valores12m
    ...
    if pg == '/sd/pluviometro.txt' or pg =='/sd/index.txt':
        contenido = contenido.replace("!!Pluviometro!!", f"{lluvia_acumulada}")
        contenido = contenido.replace("!!Clima!!", f"{clima}")
    ...
    if pg == '/sd/particulas.txt' or pg =='/sd/index.txt':
        ...
        contenido = contenido.replace("!!Particulas!!", f"{conParticulas}")
        contenido = contenido.replace("!!Particulado!!", f"{particulado}")
    ...
    if pg == '/sd/gas.txt' or pg =='/sd/index.txt':
        ...
        contenido = contenido.replace("!!Gas!!", f"{conGases}")
        contenido = contenido.replace("!!Gasificado!!", f"{gasificado}")
...
def sensado(timer):
    global temp, hum, valor_db,punto_cardinal, valor_uv, conParticulas, conGases, presion, velocidad_viento_max, velocidad_viento_actual, lluvia_acumulada,dos_minutos,clima
    oled.fill_rect(0, 0, 128, 64, 0)
...
def enviar_datos_utn(timer):
    ...
    datos = {
        ...
        "humedad": hum,
        "presion": presion,
        "viento_velocidad": velocidad_viento_actual,
        "viento_direccion": punto_cardinal,
        "lluvia": lluvia_acumulada,
        "uv": valor_uv,
        "con_gas": conGases,
        "con_particulas": conParticulas,
        "ruido": valor_db,
        "device_id": "pico_estacion_001",
    }
    ...
    if ban_guardado == 1:
        datos_noenviado.append(datos)
        cont_guardado = cont_guardado + 1
    elif ban_guardado = 0:
        for i range (cont_guardado):
            datos = datos_noenviando[i]
            
        
    
    try:
        print(f"Enviando datos a UTN...")
        
        headers = {
            "Content-Type": "application/json",
            "User-Agent": "RaspberryPiPicoW/1.0"
        }
        ...
        if response.status_code in [200, 201, 202]:
            print("Datos enviados exitosamente a UTN")
            ...
            try:
                ...
                print(f"Respuesta texto: {response_text}")
                return True, {"message": response_text}
...
tim2 = Timer()
tim3 = Timer()
tim3.init(period=180000, callback=enviar_datos_utn)
